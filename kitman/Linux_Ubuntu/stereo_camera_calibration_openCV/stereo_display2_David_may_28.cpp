...
// Open the video file
	 cv::VideoCapture capture("./camR.avi");
	 cv::VideoCapture capture1("./camL.avi");
	...
	// current video frame
	cv::Mat frame; // foreground binary image
	cv::Mat frame1; 
	cv::Mat foreground;
	cv::Mat foreground1;
	cv::namedWindow("ImageR");
	cv::namedWindow("ImageL");
	cv::namedWindow("Extracted Foreground_R");
	cv::namedWindow("Extracted Foreground_L");

	// The Mixture of Gaussian object // used with all default parameters
	cv::BackgroundSubtractorMOG mog;
	cv::BackgroundSubtractorMOG mog1;
	bool stop(false);
	...
	while (!stop) 
	{
		...
		// show foreground 
		cv::imshow("ImageR",frame);
		cv::imshow("ImageL",frame1);

		cv::imshow("Extracted Foreground_R",foreground);
		cv::imshow("Extracted Foreground_L",foreground1);

		// introduce a delay 
		// or press key to stop 
		char kk;
		kk=cv::waitKey(100) & 255;
		if (27==kk)  //check for ESCAPE
		{
			stop= true;
		}
		else if (kk>=0)
		{
			...
			// en pausa. Esperando otra tecla para continuar
			cv::waitKey();
		}
	}
